...
class GetSortedLeagueDisplayItemsUseCaseTests {
    ...
    private val player3 = Player(name = "Lionel Messi", team = Team(name = "Barcelona", rank = 3), totalGoal = 25, isFollowed = false)
    ...
    @Test
    fun `invoke with TEAM_AND_LEAGUE_RANK sorts by team and league rank`() = runTest {
        ...
        assertEquals(LeagueDisplayItem.PlayerItem(player1, league1, null), displayItems[1], "Second item should be Haaland (rank 1+1=2)")
        assertEquals(LeagueDisplayItem.PlayerItem(player2, league1, null), displayItems[2], "Third item should be Salah (rank 2+1=3)")
        assertEquals(LeagueDisplayItem.Header(league2), displayItems[3], "Fourth item should be La Liga header")
        assertEquals(LeagueDisplayItem.PlayerItem(player3, league2, null), displayItems[4], "Fifth item should be Messi (rank 3+2=5)")
        println("Display items: $displayItems")
    }

    @Test
    fun `invoke with MOST_GOALS sorts by total goals descending`() = runTest {
        coEvery { leagueRepository.getHome() } returns flowOf(ApiResult.Success(fakeDataList))
        ...
        assertEquals(5, displayItems.size, "Should have two headers and three player items")
        assertEquals(LeagueDisplayItem.Header(league1), displayItems[0], "First item should be Premier League header")
        assertEquals(LeagueDisplayItem.PlayerItem(player1, league1, null), displayItems[1], "Second item should be Haaland (27 goals)")
        assertEquals(LeagueDisplayItem.PlayerItem(player2, league1, null), displayItems[2], "Third item should be Salah (20 goals)")
        assertEquals(LeagueDisplayItem.Header(league2), displayItems[3], "Fourth item should be La Liga header")
        assertEquals(LeagueDisplayItem.PlayerItem(player3, league2, null), displayItems[4], "Fifth item should be Messi (25 goals)")
        println("Display items: $displayItems")
    }

    @Test
    fun `invoke with AVERAGE_GOALS_PER_MATCH sorts by average goals and includes avgGoals`() = runTest {
        coEvery { leagueRepository.getHome() } returns flowOf(ApiResult.Success(fakeDataList))
        ...
        assertEquals(5, displayItems.size, "Should have two headers and three player items")
        assertEquals(LeagueDisplayItem.Header(league1), displayItems[0], "First item should be Premier League header")
        assertEquals(LeagueDisplayItem.PlayerItem(player1, league1, 27f / 38), displayItems[1], "Second item should be Haaland (27/38 goals)")
        assertEquals(LeagueDisplayItem.PlayerItem(player2, league1, 20f / 38), displayItems[2], "Third item should be Salah (20/38 goals)")
        assertEquals(LeagueDisplayItem.Header(league2), displayItems[3], "Fourth item should be La Liga header")
        assertEquals(LeagueDisplayItem.PlayerItem(player3, league2, 25f / 38), displayItems[4], "Fifth item should be Messi (25/38 goals)")
        println("Display items: $displayItems")
    }
